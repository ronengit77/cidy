...
| Happy path -> Country Data | Which projects are currently active in Kenya? | None | Should route to Programme Development with `topicArea=country_data` and search the `Country Data` folder. |
...
5. Programme Development now includes `country_data` for country-level questions about projects, proposals, requests, travel, activities, contact people, countries, years, active/current/ongoing status, target groups, and LDC/LLDC/SIDS filters; `resident_coordinator` for RC/RCO, UNCT, CCA/UNSDCF, country-level support, and CDPMO country-engagement questions; and `cd_strategy` for strategic priorities, service delivery model, service lines, request prioritization, partnerships, and the AI-enabled Capacity Development Knowledge Hub/Cidy.
