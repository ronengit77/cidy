...
| Happy path -> Programme Operations | Which projects are currently active in Kenya? | None | Should route to Programme Development with `topicArea=programme_operations` and search `Cidy-PD-Operations` operational records. |
...
| `Formulate_Response_Programme_Development.yaml` | Searches Programme Development sources, including TAG/Steering Committee materials, project evaluations, operations records for projects/proposals/requests/activities, guidelines/templates, implementing partners/grants, Resident Coordinator country engagement, DESA capacity-development strategy, and the broad Programme Development fallback. | `OnRecognizedIntent` | `AssessConfidence` |
...
5. Programme Development now includes `programme_operations` for operational Excel-record questions about projects, proposals, requests, activities, countries, years, active/current/ongoing status, target groups, and LDC/LLDC/SIDS filters; `resident_coordinator` for RC/RCO, UNCT, CCA/UNSDCF, country-level support, and CDPMO country-engagement questions; and `cd_strategy` for strategic priorities, service delivery model, service lines, request prioritization, partnerships, and the AI-enabled Capacity Development Knowledge Hub/Cidy.
